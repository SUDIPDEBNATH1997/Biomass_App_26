...
plot_id = st.text_input("Plot ID (e.g., T1P1)")
...
if st.button("🌿 Calculate & Save AGB"):
    ...
    if species == "Other" and (species_final is None or species_final.strip() == ""):
...
if st.session_state.data:
    ...
    st.download_button(
        ...
        data=csv,
        file_name='mangrove_tree_data.csv',
        mime='text/csv',
    )
    ...
    # 🌱 PLOT-LEVEL BIOMASS
    # -----------------------------
    st.write("## 🌱 Plot-wise Biomass Summary")

    plot_area = st.number_input("Plot Area (m²)", value=12.57)

    # Convert gm → ton
    df["AGB (ton)"] = df["AGB (gm)"] / 1_000_000

    # Group by Plot ID
    plot_summary = df.groupby("Plot ID").agg({
        "AGB (ton)": "sum"
    }).reset_index()

    # Calculate t/ha
    plot_summary["TAGB (t/ha)"] = plot_summary["AGB (ton)"] * (10000 / plot_area)

    st.dataframe(plot_summary)

    # Download Plot Summary
    csv_plot = plot_summary.to_csv(index=False).encode('utf-8')

    st.download_button(
        label="📥 Download Plot Summary",
        data=csv_plot,
        file_name='plot_biomass_summary.csv',
        mime='text/csv',
    )
